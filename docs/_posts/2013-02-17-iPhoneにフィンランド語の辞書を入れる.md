...
あとは、以前「[iPhoneにEPWINGの辞書を入れる]({{ site.baseurl }}{% post_url 2011-06-12-iPhoneにEPWINGの辞書を入れる %})」に書いた手順でiPhoneのEBPocket for iOSにFTPします。EBPocketで辞書グループの設定などをして、いざ検索してみると……。
...
※「[12か国語大辞典の外字マップ]({{ site.baseurl }}{% post_url 2013-02-24-12か国語大辞典の外字マップ %})」に続きを書きました。
